...
public class ChangeUserName extends ABaseFragment {


    public static void launch(ChangeUserInfoFragment from, UserInfo userInfo, int CHANGE_NAME) {
        FragmentArgs args = new FragmentArgs();

        args.add("userInfo", userInfo);

        SinaCommonActivity.launchForResult(from, ChangeUserName.class, args,CHANGE_NAME);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        ((BaseActivity) getActivity()).getSupportActionBar().setTitle("修改昵称");
    ...
    protected void layoutInit(LayoutInflater inflater, Bundle savedInstanceState) {
        ...
        editText.setSelection(userInfo.getNickname().length());//设置光标位置

        done.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=getActivity().getIntent();
                intent.putExtra("nameIntent",editText.getText().toString());
